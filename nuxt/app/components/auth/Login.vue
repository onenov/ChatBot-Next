...
<template>
  <div class="space-y-4 z-[99]">
    ...
    <div class="flex gap-4">
      <UButton v-for="(provider, key) in providers" :key="key" :loading="provider.loading" :icon="provider.icon"
        :color="provider.color" :label="provider.name" size="lg" class="w-full flex items-center justify-center"
    ...
    <UForm ref="form" :state="state" @submit="onSubmit" class="space-y-4">
      ...
      <div class="flex items-center justify-end space-x-4">
        <NuxtLink class="text-sm" to="/auth/forgot">忘记密码？</NuxtLink>
        <UButton type="submit" label="登陆" :loading="loginStatus === 'pending'" />
      </div>
    </UForm>

    <div class="text-sm">
      还没有帐户？
      <NuxtLink class="text-sm" to="/auth/register">创建一个</NuxtLink>
    </div>
  </div>
</template>
